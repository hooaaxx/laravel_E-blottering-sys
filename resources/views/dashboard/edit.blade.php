        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('dashboard.blotter.update', $blotter->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        {{-- PLACE --}}
                        <div class="p-7 bg-gray-300 border-b border-gray-200">
                            ...
                            >
                                @foreach ($brgys as $brgy)
                                    <option value="{{ $brgy->name }}" {{ $brgy->name === $blotter->barangay ? "selected" : " " }}>{{ $brgy->name }}</option>
                                @endforeach
                            </select>
